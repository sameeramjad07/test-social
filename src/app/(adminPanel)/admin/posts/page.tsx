...
export default function PostsScheduling() {
  ...
  } = api.admin.getPosts.useQuery(undefined, {
    select: (data) => data as ScheduledPost[],
  });
  ...
  const handleTogglePostStatus = (postId: string, currentStatus: string) => {
    ...
    togglePostStatus.mutate({
      postId,
      status: currentStatus === "SCHEDULED" ? "DRAFT" : "SCHEDULED",
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {posts?.filter((p: ScheduledPost) => p.status === "DRAFT").length}
            </div>
          </CardContent>
        </Card>
        <Card>
